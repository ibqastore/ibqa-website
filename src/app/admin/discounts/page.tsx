...
export default function AdminDiscounts() {
  const { discounts, setDiscounts, products } = useStore();
  ...
  return <div className="animate-fade-up"><div className={styles.pageHeader}><div><h1 className={styles.pageTitle}>Discounts</h1><p style={{ color:'var(--text-secondary)', marginTop:'.4rem' }}>Schedule a site-wide offer or limit it to selected products.</p></div><button className={styles.primaryBtn} onClick={() => { setIsAdding(true); setEditingCode(null); setFormData({ code: "", percentage: 0, productIds: [], startsAt: "", endsAt: "", active: true }); }}>Create discount</button></div>
    ...
    <div className={styles.card}><table className={styles.table}><thead><tr><th>Code</th><th>Offer</th><th>Applies to</th><th>Schedule</th><th>Status</th><th>Action</th></tr></thead><tbody>{discounts.map(d => <tr key={d.code}><td style={{ fontWeight:700 }}>{d.code}</td><td>{d.percentage}% off</td><td>{d.productIds.length ? `${d.productIds.length} selected product(s)` : 'Entire website'}</td><td>{d.startsAt || 'Now'} — {d.endsAt || 'No end date'}</td><td>{d.active ? 'Active' : 'Disabled'}</td><td><div style={{ display: 'flex', gap: '0.5rem' }}><button className={styles.actionBtn} onClick={() => setDiscounts(x => x.map(item => item.code === d.code ? {...item, active:!item.active} : item))}>{d.active ? 'Disable' : 'Enable'}</button> <button className={styles.actionBtn} onClick={() => { setEditingCode(d.code); setFormData(d); setIsAdding(true); }}>Edit</button> <button className={styles.actionBtn} style={{ color: '#ff4d4d', borderColor: '#ff4d4d' }} onClick={() => confirm("Delete discount?") && setDiscounts(x => x.filter(item => item.code !== d.code))}>Delete</button></div></td></tr>)}</tbody></table></div>
  </div>;
}
